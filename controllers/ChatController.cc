    ...
    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    std::string reply = "Sanghavi Mart bot: I can help with products, orders, and auth. LLM integration planned.";
    if (msg.find("hour") != std::string::npos) reply = "Sanghavi Mart bot: Open 9am-9pm.";
    else if (msg.find("return") != std::string::npos) reply = "Sanghavi Mart bot: Returns within 7 days with bill.";
    else if (msg.find("offer") != std::string::npos) reply = "Sanghavi Mart bot: Check /api/v1/products/search for deals.";
    Json::Value ok;
    ok["reply"] = reply;
    ok["echo"] = msg;
